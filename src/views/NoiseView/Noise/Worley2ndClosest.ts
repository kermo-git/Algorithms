...
export class Worley2ndClosest2D implements Noise2D {
    ...
    noise(x: number, y: number): number {
        ...
        let min_1st_sqr = Number.MAX_VALUE
        let min_2nd_sqr = Number.MAX_VALUE
        ...
        for (let offset_x = -1; offset_x < 2; offset_x++) {
            for (let offset_y = -1; offset_y < 2; offset_y++) {
                ...
                if (dist_sqr < min_1st_sqr) {
                    min_2nd_sqr = min_1st_sqr
                    min_1st_sqr = dist_sqr
                } else if (dist_sqr < min_2nd_sqr) {
                    min_2nd_sqr = dist_sqr
                }
            }
        }
        return Math.sqrt(min_2nd_sqr)
    }
}

export class Worley2ndClosest3D implements Noise3D {
    hash_table = new HashTable(() => {
        return {
            x: Math.random(),
            y: Math.random(),
